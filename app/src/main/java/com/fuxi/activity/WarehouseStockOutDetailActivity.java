...
/**
 * Title: StockMoveOutDetailActivity Description: 出仓单明细活动界面
 * 
 * @author LYJ
 * 
 */
public class WarehouseStockOutDetailActivity extends BaseWapperActivity implements OnItemLongClickListener {

    // 静态常量
    private static final String TAG = "WarehouseStockOutDetailActivity";
    ...
    private TextView tv_relationWarehouseName;
    private TextView tv_stockNoName;
    ...
    private String stockNo; // 出仓单号
    private String relationNo; // 出仓单对应的单号
    private String stockId; // 出仓单ID
    private String employeeId; // 经手人ID
    private String warehouseId; // 转进仓库ID
    private String relationWarehouseId; // 转出仓库ID
    private String brandId; // 品牌ID
    private String auditFlag; // 出仓单的状态(是否审核)
    ...
    private void countTotal() {
        ...
        if (LoginParameterUtil.stockInAmountSumRight) {
        ...
        // break;
            case R.id.save:
                if ("保存".equals(tv_save.getText())) {
                    ...
                    if (auditRight) {
                        ...
                        Builder dialog = new AlertDialog.Builder(WarehouseStockOutDetailActivity.this, AlertDialog.THEME_HOLO_LIGHT);
                        dialog.setTitle("提示");
                        dialog.setMessage("当前暂无审核出仓单的操作权限");
                        // 相当于点击确认按钮
            ...
            case R.id.pic:
                ...
                if (null != goodsCode && !goodsCode.isEmpty()) {
                    Intent inten = new Intent(WarehouseStockOutDetailActivity.this, PictureActivity.class);
                    inten.putExtra("goodsCode", goodsCode);
                    startActivity(inten);
                } else {
                    Toast.makeText(WarehouseStockOutDetailActivity.this, "当前图片无对应的货品编码", Toast.LENGTH_SHORT).show();
                }
                break;
            default:
                break;
                ...
                public void processData(JSONObject retObj, boolean paramBoolean) {
                    ...
                    try {
                        if (retObj.getBoolean("success")) {
                            ...
                            subList.clear();
                            Toast.makeText(WarehouseStockOutDetailActivity.this, "保存成功", Toast.LENGTH_LONG).show();
                            // 保存后按钮变成审核
                            tv_save.setText("审核");
                            sendFlag = false;
                            isAdd = false;
                        } else {
                            Toast.makeText(WarehouseStockOutDetailActivity.this, "保存失败", Toast.LENGTH_LONG).show();
                        }
                    } catch (Exception e) {
                        Toast.makeText(WarehouseStockOutDetailActivity.this, "系统错误", Toast.LENGTH_LONG).show();
                        Logger.e(TAG, e.getMessage());
                    }
    ...
    private void audit() {
        if (dataList.size() < 1) {
            Toast.makeText(WarehouseStockOutDetailActivity.this, "此单据无法审核", Toast.LENGTH_SHORT).show();
            return;
        }
        RequestVo vo = new RequestVo();
        ...
        super.getDataFromServer(vo, new DataCallback<JSONObject>() {
            ...
            public void processData(JSONObject retObj, boolean paramBoolean) {
                ...
                try {
                    if (retObj.getBoolean("success")) {
                        auditFlag = "true";
                        Toast.makeText(WarehouseStockOutDetailActivity.this, "审核成功", Toast.LENGTH_LONG).show();
                        ...
                        Toast.makeText(WarehouseStockOutDetailActivity.this, "审核失败", Toast.LENGTH_LONG).show();
                    }
                } catch (Exception e) {
                    Toast.makeText(WarehouseStockOutDetailActivity.this, "系统错误", Toast.LENGTH_LONG).show();
                    Logger.e(TAG, e.getMessage());
            ...
            public void processData(JSONObject paramObject, boolean paramBoolean) {
                try {
                    if (paramObject.getBoolean("success")) {
                        Logger.e(TAG, "删除货品成功");
                        delList.clear();
                        Toast.makeText(WarehouseStockOutDetailActivity.this, "删除货品成功", Toast.LENGTH_LONG).show();
                        tv_save.setText("审核");
                        // 重新计算数量和价格
                        countTotal();
                        sendFlag = false;
    ...
    protected void processLogic() {
        // 获取用户操作权限(新增)
        modifyRight = LoginParameterUtil.stockOutRightMap.get("ModifyRight");
        auditRight = LoginParameterUtil.stockOutRightMap.get("AuditRight");
        if (!LoginParameterUtil.stockOutUnitPriceRight) {
        ...
        // 显示右上角功能
        tv_relationWarehouseName.setText("转进仓库:");
        tv_stockNoName.setText("出仓单号:");
    ...
    protected void findViewById() {
        ...
        tv_relationWarehouseName = (TextView) findViewById(R.id.relationWarehouseName);
        tv_stockNoName = (TextView) findViewById(R.id.stockNoName);
    ...
    private void back() {
        if (null == stockId || "".equals(stockId)) {
            ...
            // 点击返回时返回上一页面并刷新
            Intent intent = new Intent(WarehouseStockOutDetailActivity.this, WarehouseStockOutActivity.class);
    ...
    private void saveOrNot() {
        if (sendFlag) {
            ...
            dialog.setMessage("出仓单尚未保存,确定要返回吗？");
            // 相当于点击确认按钮
            dialog.setPositiveButton("确认", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
    ...
    private void showpopWinShare() {
        ...
        if (popWinShare == null) {
            ...
            popWinShare = new PopWinShare(WarehouseStockOutDetailActivity.this, paramOnClickListener);
        ...
        public void onClick(View v) {
            ...
            // 切换装箱/散件
                ...
                case R.id.layout_remark:
                    popWinShare.dismiss();
                    Intent intent = new Intent(WarehouseStockOutDetailActivity.this, RemarkActivity.class);
                ...
                case R.id.layout_print:
                    popWinShare.dismiss();
                    Intent intents = new Intent(WarehouseStockOutDetailActivity.this, PrintActivity.class);
                    intents.putExtra("id", stockId);
                    intents.putExtra("tableName", "stock");
                    intents.putExtra("docType", "出仓单");
                ...
                case R.id.layout_barcode_verification:
                    popWinShare.dismiss();
                    Intent inte = new Intent(WarehouseStockOutDetailActivity.this, BarcodeVerificationActivity.class);
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
        ...
        if (modifyRight) {
            ...
            Builder dialog = new AlertDialog.Builder(WarehouseStockOutDetailActivity.this, AlertDialog.THEME_HOLO_LIGHT);
            dialog.setTitle("提示");
            dialog.setMessage("当前暂无修改出仓单的操作权限");
            // 相当于点击确认按钮
            dialog.setPositiveButton("确认", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {}
