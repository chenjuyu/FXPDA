...
public class PurchaseOrderActivity extends BaseWapperActivity implements OnRefreshListener {
    private static final String TAG = "PurchaseOrderActivity";
    private static final String url = "/purchaseOrder.do?getPurchaseOrder";
    ...
    private PurchaseOrderActivity.TouchListener tl = new PurchaseOrderActivity.TouchListener();
    ...
    protected void onHeadRightButton(View v) {
        // 获取用户操作权限(新增)
        boolean addRight = LoginParameterUtil.purchaseOrderRightMap.get("AddRight");
        if (addRight) {
            // 新增销售发货单(散件)
            addItem();
        } else {
            Builder dialog = new AlertDialog.Builder(PurchaseOrderActivity.this, AlertDialog.THEME_HOLO_LIGHT);
            dialog.setTitle("提示");
            dialog.setMessage("当前暂无新增采购收货单的操作权限");
            // 相当于点击确认按钮
    ...
    protected void processLogic() {
        if (NetUtil.hasNetwork(getApplicationContext())) {
            if (LoginParameterUtil.online) {
                // 获取用户操作权限(浏览)
                boolean browseRight = LoginParameterUtil.purchaseOrderRightMap.get("BrowseRight");
                if (browseRight) {
                    ...
                    Builder dialog = new AlertDialog.Builder(PurchaseOrderActivity.this, AlertDialog.THEME_HOLO_LIGHT);
                    dialog.setTitle("提示");
                    dialog.setMessage("当前暂无浏览采购收货单的操作权限");
                    // 相当于点击确认按钮
                    dialog.setPositiveButton("确认", new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                ...
                AlertDialog.Builder dialog = new AlertDialog.Builder(PurchaseOrderActivity.this, AlertDialog.THEME_HOLO_LIGHT);
        ...
        switch (v.getId()) {
            ...
            case R.id.search:
                if ((beginDate != null && endDate == null) || (beginDate == null && endDate != null)) {
                    Toast.makeText(PurchaseOrderActivity.this, "日期范围不完整", Toast.LENGTH_SHORT).show();
                } else {
                    alertDialog.dismiss();
                    // 获取数据
                    getData(true, type);
           ...
           switch (v.getId()) {
               case R.id.no:
                   if (event.getAction() == MotionEvent.ACTION_DOWN) {
                       reset();
                       Intent intent = new Intent(PurchaseOrderActivity.this, SelectActivity.class);
                       intent.putExtra("selectType", "selectPurchaseOrderNo");
               ...
               case R.id.begin_date:
                   if (event.getAction() == MotionEvent.ACTION_DOWN) {
                       ...
                       DatePickerDialog pickerDialog = new DatePickerDialog(PurchaseOrderActivity.this, AlertDialog.THEME_HOLO_LIGHT, dateListener, Tools.year, Tools.month, Tools.day);
            ...
            public void processData(JSONObject retObj, boolean paramBoolean) {
                try {
                    ...
                    if (retObj.getBoolean("success")) {
                        if (PurchaseOrderActivity.this.refresh) {
                            dataList.clear();
                        }
                        ...
                        if (PurchaseOrderActivity.this.type == 1) {
                            ...
                            if (array.length() == 0) {
                                Toast.makeText(PurchaseOrderActivity.this, "已经到达最后一页", Toast.LENGTH_SHORT).show();
                            }
                        } else if (PurchaseOrderActivity.this.type == 2) {
                    ...
                    Toast.makeText(PurchaseOrderActivity.this, "系统错误", Toast.LENGTH_LONG).show();
